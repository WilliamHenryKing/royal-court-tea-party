...
// ============================================
// STRING LIGHTS - Decorative lights between buildings
// ============================================

export const stringLightBulbs = [];

// Create a catenary curve (hanging string shape)
function createCatenaryPoints(start, end, sag = 2, segments = 20) {
  const points = [];
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    // Catenary-like sag using parabola
...
function createStringLight(startPos, endPos, bulbCount = 8) {
  const group = new THREE.Group();

  // Create the string (wire)
  const curvePoints = createCatenaryPoints(startPos, endPos, 1.5, 30);
  const curve = new THREE.CatmullRomCurve3(curvePoints);
  const wireGeo = new THREE.TubeGeometry(curve, 30, 0.02, 8, false);
  ...
  const bulbColors = [0xfff9c4, 0xffe082, 0xffcc80, 0xffab91, 0xf8bbd9];
  ...
  for (let i = 1; i < bulbCount; i++) {
    ...
    bulb.position.y -= 0.15; // Hang below wire
...
export function createStringLights() {
  // Define connection points for string lights
  // These connect various spots around the plaza
  const lightStrings = [
    // Central plaza connections
    { start: { x: 0, y: 5, z: -5 }, end: { x: 5, y: 4.5, z: 0 }, bulbs: 6 },
    { start: { x: 5, y: 4.5, z: 0 }, end: { x: 0, y: 5, z: 5 }, bulbs: 6 },
    { start: { x: 0, y: 5, z: 5 }, end: { x: -5, y: 4.5, z: 0 }, bulbs: 6 },
    { start: { x: -5, y: 4.5, z: 0 }, end: { x: 0, y: 5, z: -5 }, bulbs: 6 },

    // Outer ring connections (higher up)
    { start: { x: -10, y: 6, z: -10 }, end: { x: 10, y: 6, z: -10 }, bulbs: 10 },
    { start: { x: 10, y: 6, z: -10 }, end: { x: 10, y: 6, z: 10 }, bulbs: 10 },
    { start: { x: 10, y: 6, z: 10 }, end: { x: -10, y: 6, z: 10 }, bulbs: 10 },
    { start: { x: -10, y: 6, z: 10 }, end: { x: -10, y: 6, z: -10 }, bulbs: 10 }
  ];

  lightStrings.forEach(({ start, end, bulbs }) => {
    const startVec = new THREE.Vector3(start.x, start.y, start.z);
    const endVec = new THREE.Vector3(end.x, end.y, end.z);
    const stringLight = createStringLight(startVec, endVec, bulbs);
    scene.add(stringLight);
  });
}

// Update string light bulbs (gentle flicker)
export function updateStringLights(time) {
  stringLightBulbs.forEach(bulb => {
    const data = bulb.userData;
    // Gentle warm flicker
    const flicker = Math.sin(time * data.flickerSpeed + data.phaseOffset) * 0.15;
    bulb.material.emissiveIntensity = data.baseEmissive + flicker;
  });
}
